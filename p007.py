...
class StreamlinedBusMonitorDashboard:
    ...
    def create_test_case_risk_analysis(self):
        """
        Create comprehensive test case risk analysis showing:
        - Test cases with most bus flips
        - Station/save combos affected per test case (INCLUDING unit_id)
        - Time ranges of flips
        - Requirements at risk per test case
        
        COUNTING RULES:
        - Each flip is uniquely identified by (unit_id, station, save, msg_type, timestamp)
        - Each flip can be counted for MULTIPLE test case groups if they overlap in time
        - Each flip is counted ONCE per test case group (even if that group has overlapping instances)
        - Station/save combos now show unit_id for clarity
        """
        if self.df_test_cases is None or self.df_test_cases.empty:
        ...
        # Group test cases by base name
        grouped_test_cases = {}
        for _, tc in self.df_test_cases.iterrows():
            base_name = self.group_test_case_id(tc['test_case_id'])
        ...
        for base_name, test_case_instances in grouped_test_cases.items():
            # Track UNIQUE flips for THIS test case group only
            unique_flips_for_this_group = set()
            
            # NOW INCLUDING UNIT_ID in the key
            unit_station_save_flips = {}  # Changed from station_save_flips
            msg_type_flips = {}
            time_ranges = []
            requirements_at_risk = set()
            
            # Track which units this test case ran on
            units_tested = set()
            ...
            for tc_instance in test_case_instances:
                ...
                # Include unit_id in the key for better visibility
                unit_station_save_key = f"{tc_instance['unit_id']}/{tc_instance['station']}/{tc_instance['save']}"
                
                # Process each flip found in this instance
                for _, flip in matching_flips.iterrows():
                    # Create unique flip identifier for THIS test case group
                    flip_id = (
                        str(flip['unit_id']),
                        str(flip['station']),
                        str(flip['save']),
                ...
                if len(matching_flips) > 0:
                    ...
                    time_ranges.append({
                        ...
                        'instance_start': tc_instance['timestamp_start'],
                        'instance_end': tc_instance['timestamp_end']
                    })
            
            # Find requirements at risk for this test case group
            ...
            if total_flips > 0:
                # Sort unit/station/save combos by flip count
                sorted_combos = sorted(unit_station_save_flips.items(), key=lambda x: x[1], reverse=True)
                
                # Sort message types by flip count
                sorted_msg_types = sorted(msg_type_flips.items(), key=lambda x: x[1], reverse=True)
                
                # Merge overlapping time ranges
                merged_time_ranges = self._merge_overlapping_time_ranges(time_ranges)
                
                # Format time ranges
                time_range_strs = []
                for tr in merged_time_ranges[:5]:
                ...
                risk_analysis.append({
                    'test_case': base_name,
                    'total_bus_flips': total_flips,
                    'units_tested': ', '.join(sorted(units_tested)),  # NEW: Show which units
                    'num_units': len(units_tested),  # NEW: Count of units
                    'instances_run': len(test_case_instances),
                    'avg_flips_per_run': round(total_flips / len(test_case_instances), 1),
                    'top_location': sorted_combos[0][0] if sorted_combos else '',  # Changed name
                    'top_location_flips': sorted_combos[0][1] if sorted_combos else 0,
                    'all_locations': ', '.join([f"{k}({v})" for k, v in sorted_combos[:5]]),  # Changed name
                    'top_msg_types': ', '.join([f"{k}({v})" for k, v in sorted_msg_types[:5]]),
                    'time_ranges': ' | '.join(time_range_strs) if time_range_strs else 'N/A',
        ...
        <script>
            ...
            function drawRiskCharts() {{
                ...
                const filteredRiskData = testCaseRiskData.filter(tc => {{
                    const testCaseFilter = document.getElementById('globalTestCaseFilter').value;
                    return !testCaseFilter || tc.test_case === testCaseFilter;
                }});
                
                // Top test cases at risk
                ...
                Plotly.newPlot('riskOverviewChart', [{{
                    ...
                    hovertemplate: '%{{x}}<br>Total Flips: %{{y}}<br>%{{text}}<extra></extra>'
                }}], {{
                ...
                topRisk.forEach(tc => {{
                    if (tc.station_save_combos) {{
                        ...
                        combos.forEach(combo => {{
                            ...
                            if (match) {{
                                stationSaveData.push({{
                                    testCase: tc.test_case,
                                    combo: match[1],
                                    flips: parseInt(match[2])
                                }});
                            }}
                        }});
                    }}
                ...
                if (stationSaveData.length > 0) {{
                    // Group by combo and sum
                    const comboTotals = {{}};
                    stationSaveData.forEach(d => {{
                        if (!comboTotals[d.combo]) {{
                            comboTotals[d.combo] = 0;
                        }}
                        comboTotals[d.combo] += d.flips;
                    }});
                    
                    const sortedCombos = Object.entries(comboTotals)
                        .sort((a, b) => b[1] - a[1])
                        .slice(0, 15);
                    
                    Plotly.newPlot('riskStationSaveChart', [{{
                        x: sortedCombos.map(d => d[0]),
                        y: sortedCombos.map(d => d[1]),
                        type: 'bar',
                        marker: {{ color: '#e74c3c' }}
                    }}], {{
                        margin: {{ t: 10, b: 100, l: 60, r: 20 }},
                ...
                Plotly.newPlot('riskRequirementsChart', [{{
                    ...
                    hovertemplate: '%{{x}}<br>Requirements at Risk: %{{y}}<br>%{{text}}<extra></extra>'
                }}], {{
                    margin: {{ t: 10, b: 120, l: 60, r: 20 }},
                    xaxis: {{ title: 'Test Case', tickangle: -45 }},
                ...
                filteredRiskData.forEach(risk => {{
                    ...
                    row.insertCell(2).textContent = risk.avg_flips_per_run;
                    row.insertCell(3).textContent = `${{risk.top_station_save_combo}} (${{risk.top_combo_flips}})`;
                    row.insertCell(4).textContent = risk.station_save_combos;
                    row.insertCell(5).textContent = risk.top_msg_types;
                    row.insertCell(6).textContent = `${{risk.requirements_at_risk}}: ${{risk.requirement_list}}`;
                    row.insertCell(7).textContent = risk.time_ranges;
                }});
            }}

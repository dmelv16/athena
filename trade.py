def run_analysis(self):
    ...
    # Save CSV files instead of Excel
    print(f"\nSaving analysis results to CSV files in {flagged_data_folder}...")
    
    try:
        # 1. All results with all statistics
        all_results_path = flagged_data_folder / 'all_results.csv'
        results_df.to_csv(all_results_path, index=False)
        print(f"  Saved: all_results.csv")
        
        # 2. Only flagged steady state
        if 'flagged' in results_df.columns and 'label' in results_df.columns:
            ...
            if not flagged_steady.empty:
                flagged_steady_path = flagged_data_folder / 'flagged_steady_state.csv'
                flagged_steady.to_csv(flagged_steady_path, index=False)
                print(f"  Saved: flagged_steady_state.csv")
            
            # 3. Summary statistics by test case and label
            ...
            summary_path = flagged_data_folder / 'summary_stats.csv'
            summary.to_csv(summary_path)  # Keep index as it contains groupby keys
            ...
            if dynamic_thresholds:
                ...
                if thresh_data:
                    ...
                    thresh_path = flagged_data_folder / 'dynamic_thresholds.csv'
                    thresh_df.to_csv(thresh_path, index=False)
                    print(f"  Saved: dynamic_thresholds.csv")
            ...
            if 'dc_folder' in results_df.columns:
                ...
                dc_path = flagged_data_folder / 'dc_comparison.csv'
                dc_summary.to_csv(dc_path)  # Keep index as it contains groupby keys
                print(f"  Saved: dc_comparison.csv")
        
        print(f"\nAll CSV files saved to: {flagged_data_folder}")
    ...
    print(f"\nOutputs:")
    print(f"  CSV files: {self.output_folder}/flagged_data/")

...
<body>
    <div class="container">
        ...
        <div class="chart-container">
            ...
            <div style="margin-bottom: 15px; padding: 15px; background: #f0f7ff; border-left: 4px solid #2196F3; border-radius: 4px;">
                <strong>What This Shows:</strong> Analysis of data words that changed during bus flips (errors only)<br>
                • Single Word Changes: Only one data word changed during the flip<br>
                • Multi Word Changes: Multiple data words changed together (more severe issue)<br>
                • Error Patterns: Shows the actual value transitions that occurred
            </div>
            
            <div id="dataWordChart"></div>
            
            <div style="margin-top: 20px;">
                <h4>Data Word Issues by Message Type</h4>
        ...
        <div class="chart-container">
            <div class="chart-title">Bus Flip Timing Analysis</div>
            <div id="flipTimingChart"></div>
            
            <div class="filters" style="margin-top: 20px;">
                <div class="filter-group">
                    <label for="timingStationFilter">Station:</label>
                    <select id="timingStationFilter" onchange="updateTimingChart()">
                        <option value="">All Stations</option>
                    </select>
                </div>
                <div class="filter-group">
                    <label for="timingMsgFilter">Message Type:</label>
                    <select id="timingMsgFilter" onchange="updateTimingChart()">
                        <option value="">All Message Types</option>
                    </select>
                </div>
                <div class="filter-group">
                    <label for="timingSaveFilter">Save:</label>
                    <select id="timingSaveFilter" onchange="updateTimingChart()">
                        <option value="">All Saves</option>
                    </select>
                </div>
            </div>
    ...
    <script>
        ...
        function drawCharts() {{
            ...
            // Data Word Chart - Simple bar chart
            drawDataWordChart();
        ...
        // Simplified Data Word Chart
        function drawDataWordChart() {{
            if (dataWordData.length > 0) {{
                // Sort and take top 20
                const sortedData = dataWordData.sort((a, b) => b.total_issues - a.total_issues).slice(0, 20);
                ...
                Plotly.newPlot('dataWordChart', [{{
                    ...
                    marker: {{ 
                        ...
                        colorscale: [
                            [0, '#4CAF50'],    // Green for mostly single word
                            [0.5, '#ff9800'],  // Orange for mixed
                            [1, '#f44336']     // Red for mostly multi word
                        ],
                        showscale: true,
                        colorbar: {{
                    ...
                    text: sortedData.map(d => 
                        `Single: ${{d.single_word_changes || 0}}<br>Multi: ${{d.multi_word_changes || 0}}<br>Speed: ${{d.avg_flip_speed_ms || 0}}ms`
                    ),
                    hovertemplate: '%{{x}}<br>Total Issues: %{{y}}<br>%{{text}}<extra></extra>'
                }}], {{
                ...
                sortedData.forEach(d => {{
                    ...
                    row.insertCell(5).textContent = `${{d.avg_flip_speed_ms || 0}}`;
                    ...
                    if (patterns.length > 80) {{
                        patternsCell.textContent = patterns.substring(0, 80) + '...';
        ...
        // Bus Flip Timing Analysis
        function initializeTimingFilters() {{
            // Station filter
            const stations = [...new Set(allData.map(d => d.station))].filter(Boolean).sort();
            const stationFilter = document.getElementById('timingStationFilter');
            ...
            // Message type filter
            const msgTypes = [...new Set(allData.map(d => d.msg_type))].filter(Boolean).sort();
            const msgFilter = document.getElementById('timingMsgFilter');
            msgTypes.forEach(type => {{
                const option = document.createElement('option');
                option.value = type;
                option.textContent = type;
                msgFilter.appendChild(option);
            }});
            
            // Save filter
            const saves = [...new Set(allData.map(d => d.save))].filter(Boolean).sort();
            const saveFilter = document.getElementById('timingSaveFilter');
            saves.forEach(save => {{
                const option = document.createElement('option');
                option.value = save;
                option.textContent = save;
        ...
        function updateTimingChart() {{
            const stationFilter = document.getElementById('timingStationFilter').value;
            const msgFilter = document.getElementById('timingMsgFilter').value;
            const saveFilter = document.getElementById('timingSaveFilter').value;
            
            // Filter data
            const filteredTimingData = allData.filter(d => {{
                return (!stationFilter || d.station === stationFilter) &&
                       (!msgFilter || d.msg_type === msgFilter) &&
                       (!saveFilter || d.save === saveFilter);
            }});
            
            drawFlipTimingChart(filteredTimingData);
        }}
        
        function drawFlipTimingChart(data = allData) {{
            if (data.length === 0) {{
                document.getElementById('flipTimingChart').innerHTML = 
                    '<div style="text-align: center; padding: 40px;">No data matching filters</div>';
                return;
            }}
            
            // Extract timing data (convert to milliseconds)
            const timings = data.map(d => {{
                const ms = d.timestamp_diff_ms || (d.timestamp_diff * 1000);
                return {{
                    value: ms,
                    label: `${{d.station}}-${{d.msg_type || 'unknown'}}-${{d.save}}`
                }};
            }}).filter(d => d.value && d.value < 100); // Only flips under 100ms
            
            // Create histogram
            Plotly.newPlot('flipTimingChart', [{{
                x: timings.map(t => t.value),
                type: 'histogram',
                nbinsx: 50,
                marker: {{
                    color: '#2196F3',
                ...
                hovertemplate: 'Speed Range: %{{x}}ms<br>Count: %{{y}}<extra></extra>'
            }}], {{
                margin: {{ t: 10, b: 60, l: 60, r: 20 }},
                xaxis: {{ 
                    title: 'Flip Speed (milliseconds)',
                    range: [0, 100]
                }},
                yaxis: {{ title: 'Number of Flips' }},
                height: 350,
                bargap: 0.05,
                annotations: [{{
                    x: data.reduce((sum, d) => sum + (d.timestamp_diff_ms || d.timestamp_diff * 1000), 0) / data.length,
                    y: 0,
                    xref: 'x',
                    yref: 'paper',
                    text: `Avg: ${{(data.reduce((sum, d) => sum + (d.timestamp_diff_ms || d.timestamp_diff * 1000), 0) / data.length).toFixed(2)}}ms`,
                    showarrow: true,
                    arrowhead: 2,
                    ax: 0,
                    ay: -40
        ...
        // Remove the complex data word analysis functions and keep only necessary ones
        
        // Initialize on page load
        initializeFilters();
        initializeTimingFilters();
        drawCharts();
    </script>
</body>
</html>

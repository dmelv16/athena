def create_test_case_risk_analysis(self):
    ...
    - Combined test cases (e.g., QS-3000&PW-0007) are split into separate entries
    """
    ...
    for _, tc in self.df_test_cases.iterrows():
        ...
        # Split combined test cases
        if '&' in base_name:
            split_names = base_name.split('&')
            for split_name in split_names:
                split_name = split_name.strip()
                if split_name not in grouped_test_cases:
                    grouped_test_cases[split_name] = []
                grouped_test_cases[split_name].append(tc)
        else:
    ...
    for base_name, test_case_instances in grouped_test_cases.items():
        ...
        for tc_instance in test_case_instances:
            ...
            # Track time ranges with unit/station/save info
        ...
        if total_flips > 0:
            ...
            # Format time ranges - each on separate line
            time_range_strs = []
            for tr in time_ranges[:10]:  # Limit to 10 to avoid overwhelming
    ...
    <body>
        ...
        <script>
            ...
            function drawRiskCharts() {{
                ...
                // Station/Save impact - FIXED
                const stationSaveImpact = {{}};
                
                topRisk.forEach(tc => {{
                    if (tc.station_save_combos) {{
                        // Parse the station/save combos from format "L1/4(10), L2/3(5)"
                        ...
                        combos.forEach(combo => {{
                            ...
                            if (match) {{
                                ...
                                if (!stationSaveImpact[tc.test_case]) {{
                ...
                // Create stacked bar chart for station/save impact
                if (Object.keys(stationSaveImpact).length > 0) {{
                    ...
                    const traces = [];
                    const locationArray = Array.from(allLocations).slice(0, 20); // Limit to top 20 locations
                ...
                // Requirements at risk chart - FIXED
                ...
                if (reqRiskData.length > 0) {{
                    Plotly.newPlot('riskRequirementsChart', [{{
                        ...
                        hovertemplate: '%{{x}}<br>Requirements at Risk: %{{y}}<br>%{{text}}<br>Reqs: ' + 
                                    reqRiskData.map(d => d.requirement_list).join(',') + '<extra></extra>',
                ...
                filteredRiskData.forEach(risk => {{
                    ...
                    row.cells[7].innerHTML = timeRangesFormatted; // Use innerHTML for line breaks
                }});
            }}

#!/usr/bin/env python3
"""
Enhanced Bus Monitor Analysis - Version 16
Includes:
...
5. FIX: Corrected test case risk deduplication for station/save counts
6. FIX: Loaded requirement mapping before analysis to populate risk table
"""
...
class StreamlinedBusMonitorDashboard:
            ...
            for tc_instance in test_case_instances:
                ...
                for _, flip in matching_flips.iterrows():
                    ...
                    if flip_id not in unique_flips_for_this_group:
                        ...
                        # FIX: MOVED INSIDE the 'if' block to ensure deduplication
                        # Track location
    ...
    def run_analysis(self):
        ...
        # FIX: Load requirement-testcase mapping FIRST
        ...
        if self.df_test_cases is not None and not self.df_test_cases.empty:
            ...
            self.create_test_case_risk_analysis() # <-- Mapping is now available
    ...
    <body>
        ...
        <script>
            ...
            const uniqueTestCases = {uniqueTestCases};
            ...
            function drawRiskCharts() {{
                ...
                Plotly.newPlot('riskOverviewChart', [{{
                    ...
                    // FIX: Correctly use customdata in hovertemplate
                    customdata: topRisk.map(d => d.units_tested),
                    hovertemplate: '<b>%{{x}}</b><br>Total Flips: %{{y}}<br>%{{text}}<br>Units: %{{customdata}}<extra></extra>'
                }}], {{
                ...
                if (Object.keys(stationSaveImpact).length > 0) {{
                    ...
                    locationArray.forEach(location => {{
                        ...
                        traces.push({{
                            ...
                            name: location,
                            type: 'bar',
                            // FIX: Improved hover template
                            hovertemplate: '<b>Location: %{{fullData.name}}</b><br>Test Case: %{{x}}<br>Flips: %{{y}}<extra></extra>'
                        }});
                    }});
                ...
                filteredRiskData.forEach(risk => {{
                    ...
                    // FIX: Use the correct variable 'risk.time_ranges'
                    row.insertCell(7).textContent = risk.time_ranges || 'N/A';
                }});
            }}
                ...
                if (universalReqsWithFlips.length > 0) {{
                    ...
                    const reqData = Object.entries(reqSummary).map(([name, data]) => {{
                        ...
                        return {{
                            ...
                            hoverText: `<b>${{name}}</b><br>Total Flips: ${{data.flipCount}}<br><br>Top Locations:<br>${{stationSaveInfo}}`
                        }};
                    ...
                    Plotly.newPlot('universalRequirementsChart', [{{
                        ...
                        type: 'bar',
                        marker: {{ color: '#e74c3c' }},
                        // FIX: Use customdata for hover, not conflicting hovertext/text
